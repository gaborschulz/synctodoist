...
def test_get_section_by_pattern(synced_todoist, section_added):
    section = synced_todoist.find_section(pattern='fixture_section_added')
    assert section.name == section_added.name


def test_delete_section_by_model(synced_todoist, section_added_no_teardown):
    synced_todoist.delete_section(section=section_added_no_teardown)
    synced_todoist.commit()
    with pytest.raises(TodoistError):
        synced_todoist.get_section(section_id=section_added_no_teardown.id)


def test_delete_section_by_id(synced_todoist, section_added_no_teardown):
    synced_todoist.delete_section(section_id=section_added_no_teardown.id)
    synced_todoist.commit()
    with pytest.raises(TodoistError):
        synced_todoist.get_section(section_id=section_added_no_teardown.id)
